...
P_el_rated                      = 5e6;
Omega_g_rated                   = 12.1*r_GB;
M_g                             = P_el_rated/(eta_el*Omega_g_rated*2*pi/60);                                    % constant torque for 16 m/s TODO!!!
% TSR = Omega_g_rated*2*pi/60*Parameter.Turbine.R/16;
% Cp = 0.1693;
% beta = reverseinterp2(TSR,Cp,lambda,theta,c_P);
beta = 12.05745;
theta                           = deg2rad(beta);                           % constant pitch for 16 m/s TODO!!!
...
x(1,1)      = Omega_g_rated/60*2*pi/r_GB;     % rotor speed               [rad/s]
